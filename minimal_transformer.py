    ...
    def forward(self, Q, K, V, mask=None):
        ...
        if mask is not None:
            ...
            if mask.dim() == 4 and mask.size(1) == 1:
                # 扩展掩码以匹配多头注意力的形状 [batch_size, n_heads, seq_len_q, seq_len_k]
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, enc_output, look_ahead_mask=None, padding_mask=None):
        attn_output = self.masked_mha(x, x, x, look_ahead_mask)
        x = self.layernorm1(x + attn_output)
        attn_output = self.encoder_decoder_mha(x, enc_output, enc_output, padding_mask)
...
class Transformer(nn.Module):
    def __init__(self, src_vocab_size, tgt_vocab_size, d_model=16, n_heads=2, n_layers=1, d_ff=32, max_seq_len=50):
    ...
    def generate_mask(self, src, tgt):
        ...
        # 编码器-解码器注意力掩码：形状为 [batch_size, 1, tgt_seq_len, src_seq_len]
        ...
        return src_mask, tgt_mask, enc_dec_mask
    
    def forward(self, src, tgt):
        # 生成掩码
        src_mask, tgt_mask, enc_dec_mask = self.generate_mask(src, tgt)
        
        # 编码器前向传播
        enc_emb = self.positional_encoding(self.encoder_embedding(src))
        enc_output = enc_emb
        for enc_layer in self.encoder_layers:
            enc_output = enc_layer(enc_output, src_mask)
        
        # 解码器前向传播
        dec_emb = self.positional_encoding(self.decoder_embedding(tgt))
        dec_output = dec_emb
        for dec_layer in self.decoder_layers:
            dec_output = dec_layer(dec_output, enc_output, tgt_mask, enc_dec_mask)
