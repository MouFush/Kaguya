...
<body>
    <div class="container">
        ...
        <div class="chat-container" id="chatContainer">
            <div class="message assistant"><div class="message-content">你好！我是Qwen3.5-9B助手，有什么可以帮助你的吗？</div></div>
        </div>
        <div class="loading" id="loading">正在思考中...</div>
        <div class="settings">
            <div class="setting-item"><label>温度:</label><input type="range" id="temperature" min="0.1" max="2.0" step="0.1" value="0.7"><span id="tempValue">0.7</span></div>
            <div class="setting-item"><label>Top-p:</label><input type="range" id="topP" min="0.1" max="1.0" step="0.05" value="0.9"><span id="topPValue">0.9</span></div>
            <div class="setting-item"><label>最大长度:</label><input type="range" id="maxTokens" min="64" max="2048" step="64" value="512"><span id="maxTokensValue">512</span></div>
    ...
    <script>
        ...
        function addMessage(role, content) { const container = document.getElementById('chatContainer'); const msgDiv = document.createElement('div'); msgDiv.className = 'message ' + role; msgDiv.innerHTML = '<div class="message-content">' + content + '</div>'; container.appendChild(msgDiv); container.scrollTop = container.scrollHeight; }
        function clearChat() { history = []; document.getElementById('chatContainer').innerHTML = '<div class="message assistant"><div class="message-content">对话已清空。有什么新问题吗？</div></div>'; }
        async function sendMessage() {
            const input = document.getElementById('userInput'); const message = input.value.trim(); if (!message) return;
            const sendBtn = document.getElementById('sendBtn'); const loading = document.getElementById('loading');
            sendBtn.disabled = true; input.value = ''; addMessage('user', message); loading.classList.add('active');
            const temperature = parseFloat(document.getElementById('temperature').value);
            const topP = parseFloat(document.getElementById('topP').value);
            const maxTokens = parseInt(document.getElementById('maxTokens').value);
            try {
                const response = await fetch('/chat', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ message: message, history: history, temperature: temperature, top_p: topP, max_tokens: maxTokens }) });
            ...
            loading.classList.remove('active'); sendBtn.disabled = false; input.focus();
        }
    </script>
</body>
</html>
...
def chat(message, history, temperature, top_p, max_tokens):
    ...
    with torch.no_grad():
        outputs = model.generate(**inputs, max_new_tokens=max_tokens, do_sample=True, temperature=temperature, top_p=top_p, pad_token_id=tokenizer.eos_token_id)
