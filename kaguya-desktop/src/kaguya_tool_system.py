...
class BashTool(Tool):
    ...
    DANGEROUS_PATTERNS = [
        "rm -rf /", "rm -rf ~", "rm -rf /*",
        "format c:", "del /s /q c:\\", "del /f /s /q C:",
        ":(){ :|:& };:",
    ...
    DANGEROUS_REGEX_PATTERNS = [
        (r'\brm\s+-[a-zA-Z]*r[a-zA-Z]*f[a-zA-Z]*\s+/', "rm -rf on root"),
        (r'\bdd\s+if=/dev/(zero|random|urandom)', "dd from device"),
        (r'\b(sudo|runas)\s+', "privilege escalation"),
        (r'\bchmod\s+(-R\s+)?777\s+/', "chmod 777 on root"),
        (r':\(\)\{.*\}', "fork bomb"),
        (r'\b(curl|wget)\s+.*\|\s*(bash|sh|python|perl|ruby)', "remote script execution"),
    ]
    ...
    def check_permissions(self, args, context):
        ...
        for regex_pattern, description in self.DANGEROUS_REGEX_PATTERNS:
            if re.search(regex_pattern, cmd, re.IGNORECASE):
                return PermissionResult(
                    decision=PermissionDecision.DENY,
                    message=f"Dangerous command pattern: {description}",
                )
        return super().check_permissions(args, context)
            ...
            for filepath in files:
                ...
                try:
                    ...
                    if file_size > max_file_size:
                        results.append(
                            f"{filepath}: [SKIPPED] File too large ({file_size // (1024*1024)}MB > {max_file_size // (1024*1024)}MB)"
                        )
                        continue
