...
class JWTManager:
    ...
    def _load_or_create_secret(self) -> str:
        try:
            if os.path.exists(self._JWT_KEY_FILE):
                with open(self._JWT_KEY_FILE, 'r') as f:
                    key = f.read().strip()
        ...
        try:
            os.makedirs(os.path.dirname(self._JWT_KEY_FILE), exist_ok=True)
            with open(self._JWT_KEY_FILE, 'w') as f:
                f.write(key)
        except Exception:
            pass
        return key
    ...
    def _init_revoked_db(self):
        try:
            import sqlite3
            os.makedirs(os.path.dirname(self._REVOKED_DB), exist_ok=True)
            conn = sqlite3.connect(self._REVOKED_DB)
    ...
    def _load_revoked_tokens(self):
        try:
            import sqlite3
            cutoff = (datetime.utcnow() - timedelta(days=self._refresh_expiry_days + 1)).timestamp()
            conn = sqlite3.connect(self._REVOKED_DB)
    ...
    def refresh_access_token(self, refresh_token: str) -> Optional[Tuple[str, str]]:
        try:
            ...
            new_access, new_refresh = self.generate_token(user_id, role)
            ...
            if payload and payload.get("jti"):
                ...
                try:
                    ...
                    conn.execute('INSERT OR IGNORE INTO revoked_tokens (jti, revoked_at) VALUES (?, ?)',
                                 (payload["jti"], datetime.utcnow().timestamp()))
                    conn.commit()
                    conn.close()
                except Exception:
                    pass
...
class AccountManager:
    ...
    def create_account(self, username: str, password: str, email: str = "",
        ...
        with self._lock:
            if username.lower() in self._username_index:
                return None, "用户名已存在"
            if email and email.lower() in self._email_index:
                return None, "邮箱已被使用"
    ...
    def authenticate(self, username: str, password: str,
        ...
        with self._lock:
            ...
            account = self._accounts.get(uid) if uid else None
    ...
    def change_password(self, user_id: str, old_password: str,
        ...
        for old_hash in history[-PasswordManager.PASSWORD_HISTORY_SIZE:]:
            old_salt = old_hash.split(':', 1)[0] if ':' in old_hash else account.salt
            if PasswordManager.verify_password(new_password, old_hash, old_salt):
        ...
        history.append(f"{new_salt}:{new_hash}")
    ...
    def update_account(self, user_id: str, updates: Dict[str, Any],
                       requesting_user_id: str = None) -> Tuple[bool, str]:
        with self._lock:
            account = self._accounts.get(user_id)
            if not account:
                return False, "账户不存在"
            ...
            for field_name, value in updates.items():
                if field_name in allowed_fields:
                    if field_name == 'email':
                        ...
                        if value and value.lower() in self._email_index and self._email_index[value.lower()] != user_id:
                            return False, "邮箱已被使用"
                        if old_email and old_email.lower() in self._email_index:
                            self._email_index.pop(old_email.lower(), None)
                        if value:
                            self._email_index[value.lower()] = user_id
                    setattr(account, field_name, value)

            account.updated_at = datetime.utcnow().isoformat()

        self._save_accounts()
        self._audit.log(AuthEventType.PERMISSION_CHANGE, user_id=requesting_user_id or user_id,
                        resource="account", action="update",
                        details={"target_user": user_id, "fields": list(updates.keys())})
        return True, ""
        ...
        try:
            ...
            with self._lock:
                for uid, d in data.items():
                    ...
                    acc = self._accounts[uid]
                    self._username_index[acc.username.lower()] = uid
                    if acc.email:
                        self._email_index[acc.email.lower()] = uid
        except Exception:
            pass
