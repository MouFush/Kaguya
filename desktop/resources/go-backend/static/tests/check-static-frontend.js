...
assert(index.includes("KaguyaAPI.get('/rag/documents'"), 'RAG document list must use shared API client');
assert(index.includes("KaguyaAPI.json('/rag/search'"), 'RAG search must use shared API client');
assert(index.includes("KaguyaAPI.json('/external/config'"), 'Scene provider config must use shared API client');
assert(index.includes("KaguyaAPI.json('/agent/file-tree'"), 'Sidebar IDE file tree must use shared API client');
assert(index.includes("KaguyaAPI.json('/agent/file-write'"), 'Sidebar IDE file write must use shared API client');
assert(index.includes("KaguyaAPI.json('/agent/api-status'"), 'Agent API status must use shared API client');
assert(index.includes('<option value=\"kimi\">Kimi K2.6</option>'), 'Scene provider dropdown must include Kimi');
assert(index.includes('<option value=\"minimax\">MiniMax</option>'), 'Scene provider dropdown must include MiniMax');
assert(!index.includes("fetch('/rag/documents'"), 'RAG documents must not keep naked fetch');
assert(!index.includes("fetch('/rag/search'"), 'RAG search must not keep naked fetch');
assert(!index.includes("fetch('/external/config'"), 'External config must not keep naked fetch');
assert(!index.includes("fetch('/agent/file-tree'"), 'Sidebar IDE file tree must not keep naked fetch');
assert(!index.includes("fetch('/agent/file-write'"), 'Sidebar IDE file write must not keep naked fetch');
assert(!index.includes("fetch('/agent/api-status'"), 'Agent API status must not keep naked fetch');
