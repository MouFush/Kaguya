...
`;
        ...
        function openPromptWorkbench() {
            ...
            modal.innerHTML = `
                <div class="modal-content" style="max-width:900px;">
                    <div class="modal-header">
                        <h3>🧭 专业工作台</h3>
                        <button class="modal-close" onclick="closeModal('promptWorkbenchModal')">×</button>
                    </div>
                    <div class="modal-body" style="display:grid;grid-template-columns:250px 1fr;gap:15px;max-height:70vh;">
                        <div class="workbench-sidebar">
                            <div class="sidebar-section">
                                <div class="sidebar-title">模板分类</div>
                                <div class="sidebar-items">
                                    <div class="sidebar-item active" onclick="selectWorkbenchCategory('all')">全部模板</div>
                                    <div class="sidebar-item" onclick="selectWorkbenchCategory('favorites')">⭐ 收藏模板</div>
                                    <div class="sidebar-item" onclick="selectWorkbenchCategory('recent')">🕐 最近使用</div>
                                    <div class="sidebar-item" onclick="selectWorkbenchCategory('custom')">🔧 自定义模板</div>
                                </div>
                            </div>
                            <div class="sidebar-section">
                                <div class="sidebar-title">场景分类</div>
                                <div class="sidebar-items">
                                    <div class="sidebar-item" onclick="selectWorkbenchCategory('code')">💻 代码开发</div>
                                    <div class="sidebar-item" onclick="selectWorkbenchCategory('doc')">📄 文档写作</div>
                                    <div class="sidebar-item" onclick="selectWorkbenchCategory('data')">📊 数据分析</div>
                                    <div class="sidebar-item" onclick="selectWorkbenchCategory('design')">🎨 设计创意</div>
                                </div>
                            </div>
                        </div>
                        <div class="workbench-main">
                            <div class="workbench-toolbar">
                                <input class="project-input" placeholder="搜索模板..." style="flex:1;">
                                <button class="project-mini-btn" onclick="createCustomTemplate()">+ 新建模板</button>
                            </div>
                            <div class="workbench-grid" id="workbenchTemplateGrid"></div>
                        </div>
                    </div>
                </div>
            `;
            document.body.appendChild(modal);
            loadWorkbenchTemplates();
        }

        function selectWorkbenchCategory(category) {
            document.querySelectorAll('.workbench-sidebar .sidebar-item').forEach(el => el.classList.remove('active'));
            event.target.classList.add('active');
            loadWorkbenchTemplates(category);
        }

        function loadWorkbenchTemplates(category = 'all') {
            const grid = document.getElementById('workbenchTemplateGrid');
            if (!grid) return;
            const templates = [
                { id: 'code-review', name: '代码审查', icon: '🔍', desc: '专业代码质量分析', category: 'code' },
                { id: 'doc-gen', name: '文档生成', icon: '📄', desc: '自动生成技术文档', category: 'doc' },
                { id: 'api-design', name: 'API 设计', icon: '🔌', desc: 'RESTful API 规划', category: 'code' },
                { id: 'test-gen', name: '测试生成', icon: '🧪', desc: '单元测试用例生成', category: 'code' },
                { id: 'refactor', name: '代码重构', icon: '♻️', desc: '智能代码优化建议', category: 'code' },
                { id: 'security', name: '安全审计', icon: '🔒', desc: '代码安全漏洞检测', category: 'code' },
            ];
            grid.innerHTML = templates.map(t => `
                <div class="workbench-template-card" onclick="applyWorkbenchTemplate('${t.id}')">
                    ...
                    <div class="template-actions">
                        <button class="template-action-btn" onclick="event.stopPropagation();favoriteTemplate('${t.id}')">⭐</button>
                    </div>
                </div>
            `).join('');
        }
        ...
        function loadTemplateAnalysis() {
            ...
            fetch('/templates/analysis?range=' + range)
                ...
                .then(data => {
                    if (data.success) {
                        document.getElementById('workbenchTemplateCount').textContent = data.stats.total_templates || '--';
                        document.getElementById('workbenchFavoriteCount').textContent = data.stats.favorites || '--';
                        document.getElementById('workbenchUsageCount').textContent = data.stats.usage || '--';
                        document.getElementById('workbenchCustomCount').textContent = data.stats.custom || '--';
                    }
                });
        }
        ...
        function refreshRagStats() {
            fetch('/rag/stats')
                ...
                .then(data => {
                    if (data.success) {
                        document.getElementById('ragStatDocs').textContent = data.stats.docs || 0;
                        document.getElementById('ragStatChunks').textContent = data.stats.chunks || 0;
                        document.getElementById('ragStatChars').textContent = data.stats.chars || 0;
                        document.getElementById('ragStatVectors').textContent = data.stats.vectors || 0;
                    }
                });
        }
        ...
        function openFeatureSection(name) {
            ...
            if (tabMappings.includes(name)) {
                ...
                if (target) switchTab(name, target);
                focusFeatureTarget(getFeatureTargetId(name));
